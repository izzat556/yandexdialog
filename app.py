...
# ---------- Smart Home / Discovery, Query, Action ----------
...
def yandex_dialog():
    ...
    request_id = body.get("headers", {}).get("request_id", "") or request.headers.get("X-Request-Id", "unknown")
                ...
                "devices": [
                    {
                        ...
                        "capabilities": [
                            ...
                            {"type": "devices.capabilities.color_setting", "retrievable": True, "reportable": True,
                             "parameters": {"color_model":"hsv","temperature_k":{"min":2700,"max":9000,"precision":1}}}
                        ],
                        "device_info": {"manufacturer":"Provider2","model":"hue g11","hw_version":"1.2","sw_version":"5.4"}
                    }
                ]
            }
        })
        ...
        for item in devices:
            ...
            if device_id in DEVICES:
                ...
                result_devices.append({
                    "id": device_id,
                    "capabilities":[{"type":"devices.capabilities.on_off","state":{"instance":"on","value": dev["state"]}}]
                })
            else:
...
def authorize():
    ...
    if request.method == "GET":
        ...
        if not validate_client(client_id) or redirect_uri != YANDEX_REDIRECT_URI:
    ...
    if not validate_client(client_id) or redirect_uri != YANDEX_REDIRECT_URI or username != TEST_USERNAME or password != TEST_PASSWORD:
...
def token():
    ...
    if grant_type!="authorization_code" or not validate_client(client_id,client_secret) or code not in AUTH_CODES:
        return oauth_error("invalid_request","OAuth error",400)
    code_data = AUTH_CODES[code]
    if code_data["used"] or code_data["expires_at"]<int(time.time()):
        return oauth_error("invalid_grant","Code invalid or expired",400)
...
def refresh():
    ...
    client_id = request.form.get("client_id","")
    client_secret = request.form.get("client_secret","")
    if grant_type!="refresh_token" or not validate_client(client_id,client_secret) or refresh_token not in REFRESH_TOKENS:
        return oauth_error("invalid_request","OAuth error",400)
    token_data = REFRESH_TOKENS[refresh_token]
    if token_data["expires_at"] < int(time.time()):
        return oauth_error("invalid_grant","Refresh token expired",400)
